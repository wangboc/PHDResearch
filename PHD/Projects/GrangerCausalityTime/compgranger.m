function [Fx2y,Fy2x]=compgranger(x,Nr,Nl,porder,fs,freq)
...
%[pp,cohe,Fx2y,Fy2x]=compgranger(x,1,2000,2,200,1:100);
%（两个通道的数据，几次实验，每次实验的点数，阶数，采样频率，频率范围）
% (x,Nr,Nl,porder,fs,freq)
[L,N] = size(x); %L is the number of channels, N is the total points in every channel
%[A,Z] = compcoef(x,Nr,Nl,porder);  %fitting AR model using my own code
%（两个通道的数据，几次实验，每次实验的点数，阶数）
% A 是回归模型的系数， Z 是白噪声的协方差矩阵
...
index = 0;
 for i = 1:L-1
     for j = i+1:L
         ...
         y(1,:) = x(i,:);
         y(2,:) = x(j,:);  
         [A2,Z2] = compcoef(y,Nr,Nl,porder); %fitting a model on every possible pair
         eyx = Z2(2,2) - Z2(1,2)^2/Z2(1,1); %corrected covariance
         exy = Z2(1,1) - Z2(2,1)^2/Z2(2,2);
         f_ind = 0;
         for f = freq
             f_ind = f_ind + 1;
             [S2,H2] = spectrum1(A2,Z2,porder,f,fs);
